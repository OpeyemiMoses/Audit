...
function line(label: string, value: any, unit: string = ""): string {
...
function flags(arr: string[] | undefined, label: string): string {
  if (!arr || arr.length === 0) return '- ' + label + ': None detected\n';
  return '- ' + label + ':\n' + arr.map(f => '  [!] ' + f).join('\n') + '\n';
}

function verdict(v: string | undefined): string {
  if (!v) return '';
...
function formatTokenText(raw: any): string {
...
function formatContractText(raw: any): string {
  const d = raw && raw.data ? raw.data : (raw || {});
  const intel = d.protocol_intelligence || {};
  const feedback = d.ai_feedback || {};
...
function formatWalletText(raw: any): string {
  const d = raw && raw.data ? raw.data : (raw || {});
...
function formatTransactionText(raw: any): string {
  const d = raw && raw.data ? raw.data : (raw || {});
...
function formatDecisionText(raw: any): string {
  const d = raw && raw.data ? raw.data : (raw || {});
...
function formatMarketDepthText(raw: any): string {
  const d = raw && raw.data ? raw.data : (raw || {});
...
function formatProtocolText(raw: any): string {
  const d = raw && raw.data ? raw.data : (raw || {});
...
function formatDeFiText(raw: any): string {
  const d = raw && raw.data ? raw.data : (raw || {});
...
function formatUnifiedText(raw: any): string {
